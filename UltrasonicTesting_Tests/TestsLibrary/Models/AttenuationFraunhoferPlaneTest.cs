...
namespace UltrasonicTesting_Tests
{
    ...
    public class AttenuationFraunhoferPlaneTest
    {
        ...
        {
            new AttenuationFraunhoferPlane(null, new TestObject(new Material(string.Empty, 1, 1, 1), 1));
        }
        [TestMethod]
        [ExpectedException(typeof(ArgumentNullException))]
        public void ArgumentNullExceptionTestObjectCtor()
        {
            new AttenuationFraunhoferPlane(new RoundPEC(1, new Material(string.Empty, 1, 1, 1), new AcousticWave(1, 1)), null);
        }
